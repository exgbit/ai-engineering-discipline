...
def design_diagram_required(request: ManagedRequest) -> bool:
    return bool(request.verify_params.get("require_design_diagram"))


def design_diagram_path(target: Path, request: ManagedRequest) -> Path:
    """AI 手绘设计图的统一存放位置:docs/diagrams/<slug>-design.md(自动依赖图在同目录 -impact.md)。"""
    return target / "docs" / "diagrams" / f"{slugify(request.name)}-design.md"


# 设计图占位注释的识别串:write_design_diagram 生成、design_diagram_missing 检查,必须一致
DESIGN_DIAGRAM_PLACEHOLDER = "%% TBD"

# mermaid 围栏:3+ 反引号、info 串可带属性,闭合围栏须与开头等长
MERMAID_FENCE_RE = re.compile(r"(`{3,})mermaid[^\n]*\n([\s\S]*?)\1")
# 仅有图类型声明(如 "flowchart LR")而无任何节点/边,不算已画
MERMAID_DECL_RE = re.compile(
    r"^(?:flowchart|graph|sequenceDiagram|classDiagram|stateDiagram(?:-v2)?"
    r"|erDiagram|journey|gantt|pie|mindmap|timeline)\b[\sA-Za-z]*$"
)


def _mermaid_line_is_drawn_content(line: str) -> bool:
    stripped = line.strip()
    if not stripped or stripped.startswith("%%"):
        return False
    return not MERMAID_DECL_RE.match(stripped)


def design_diagram_missing(request: ManagedRequest) -> str:
    """设计图完成度判定,返回缺失原因("" = 已画)。只认 mermaid 块里有没有真实内容
    (非注释、非纯图类型声明的行);画了就算完成,即使正文/注释里还留着 TBD 字样。
    不用 section_is_complete——设计图是自由 Markdown,字面 TBD 或未勾选清单不该误伤。"""
    diagram = design_diagram_path(request.target, request)
    diagram_rel = rel(diagram, request.target)
    if not diagram.exists():
        # 消息不放反引号:报告渲染会把 blocking reasons 整体包进单反引号 code span
        return f"design diagram missing: draw the key nodes in Mermaid at {diagram_rel} before coding"
    text = diagram.read_text(encoding="utf-8", errors="ignore")
    drawn = any(
        _mermaid_line_is_drawn_content(line)
        for _, block in MERMAID_FENCE_RE.findall(text)
        for line in block.splitlines()
    )
    if drawn:
        return ""
    if DESIGN_DIAGRAM_PLACEHOLDER in text:
        return f"design diagram incomplete: {diagram_rel} still holds its placeholder comment"
    return f"design diagram incomplete: {diagram_rel} has no Mermaid diagram content"


def design_diagram_spec_section(target: Path, request: ManagedRequest) -> str:
    """spec 里的 Design Diagram 段:只留链接,图本体在 docs/diagrams/。preset 不要求时不生成该段。"""
    if not design_diagram_required(request):
        return ""
    slug = slugify(request.name)
    return f"""
## Design Diagram

Key nodes must be drawn **before coding**. Fill `docs/diagrams/{slug}-design.md` with a Mermaid
diagram of the classes / functions this change touches and their call/dependency relations.
The auto-generated blast-radius graph in `docs/diagrams/{slug}-impact.md` can be a starting
point, but it is refreshed during verify — before any code change exists (the normal case for
new work) it is empty or missing, so draw from reading the code.
The verification gate blocks while the diagram file still holds its placeholder.
"""


def write_design_diagram(target: Path, request: ManagedRequest, force: bool, actions: list[str]) -> None:
    """生成设计图占位文件(AI 填)。占位注释以 DESIGN_DIAGRAM_PLACEHOLDER 开头,
    design_diagram_missing 按同一常量判定占位是否已删。"""
    if not design_diagram_required(request):
        return
    slug = slugify(request.name)
    content = f"""# Design Diagram: {request.name}

在实现前,用 Mermaid 画出本次改动的关键节点:受影响/新增的类、函数,以及它们的调用/依赖关系。
同目录 `{slug}-impact.md` 的自动依赖图可作参考,但它随 verify 刷新——编码前没有代码改动时
它为空或不存在,属正常,直接读现有代码画。画完删除下方占位注释。

```mermaid
{DESIGN_DIAGRAM_PLACEHOLDER}: draw the key nodes (affected/new classes, functions) and their call/dependency relations
```
"""
    safe_write(design_diagram_path(target, request), content, force, actions)
...
{design_diagram_spec_section(target, request)}
## Impact Analysis
...
def required_verify_checks(request: ManagedRequest) -> list[str]:
    ...
    if verify.get("require_build"):
        checks.append("build")
    if verify.get("require_design_diagram"):
        checks.append("design_diagram")
...
def verification_gate_details(
    ...
    # 设计图先行(preset 条件化,检查独立图文件而非 spec 段):缺失或未画 → 阻断
    if "design_diagram" in required:
        design_reason = design_diagram_missing(request)
        if design_reason:
            blocking_reasons.append(design_reason)
    ...
    if require_dc or diff_cov is not None:
        if diff_cov is None:
            # 未运行 ≠ 工具缺失,分开表述,别把用户支去装 coverage 工具
            msg = "diff-coverage required but not run: enable --run-diff-coverage (or the verify subcommand)"
            if require_dc:
                blocking_reasons.append(msg)
            else:
                uncovered_checks.append(msg)
        elif not diff_cov.get("available"):
            reason = diff_cov.get("reason", "unknown")
    ...
    if request.task in TASKS_NEEDING_TESTS and impact is not None:
        if not impact.get("available"):
            ...
            blocking_reasons.append(
                ...
                f"({changed_code_count} code file(s)) — ensure new files are tracked by git "
                f"(git add) and the language is supported by the graph"
            )
...
def humanize_blocking(item: str) -> str:
    ...
    if "design diagram" in low:
        return "The design diagram (key nodes in Mermaid, under docs/diagrams/) isn't drawn yet."
...
def write_verification_results(
    ...
    payload: dict[str, object] = {
        ...
        **gate_details,
        ...
        # nodes/edges/changed 全量图数据已落盘 impact-graph.json,这里只存门禁所需摘要,
        # 避免大仓库时 verification-results.json 把整张图再存一份
        "impact_graph": (
...
def print_actions(actions: list[str]) -> None:
    """stdout 只打关键动作;skip 行聚合成一行计数,细节在 execution-report(否则每次
    execute 刷 20+ 行 skip 全路径,red-phase / impact 计数等关键信号被淹没)。"""
    skipped = 0
    for action in actions:
        if action.startswith("skip "):
            ...
            print(action)
    if skipped:
        print(f"skipped {skipped} existing artifact(s) — details in docs/ai-engineering/execution-report.md")
...
def main() -> int:
    ...
    write_design_diagram(target, request, args.force, actions)
    ...
    if args.run_diff_coverage or request.verify_params.get("require_diff_coverage"):
        ...
        if code_changed:
            ...
            # 没改业务代码 → 无改动行可核,not-applicable 而非阻断
            # (与测试配套门禁"没改码无需配套测试"同口径;预编码阶段 verify 不该被它拦)
            diff_cov = {
                "available": True,
                "not_applicable": True,
                "uncovered_lines": {},
                "reason": "no code changes to cover",
            }
            actions.append("diff-coverage: not applicable (no code changes)")
    ...
    if request.task in TASKS_NEEDING_TESTS and not os.environ.get("AI_DISCIPLINE_GRAPH_OPTIONAL"):
        ...
        if not graph_available():
            ...
            refresh_test_index(target, actions)
            detect = detect_changes(target, since=git_diff_base(request))
            ...
            impact = {
                ...
                "slug": slugify(request.name),
                # 静态回退是正则近似:来源与原因透出到产物,不静默降级
                "graph_source": "static-fallback" if is_fallback else "knowledge-graph",
                "fallback_reason": (detect or {}).get("fallback_reason", ""),
                **graph_data,
            }
    ...
    # 非零退出时把原因回显到 stderr:别让用户翻 JSON 才知道被什么拦住
    if args.fail_on_verify_failure and not gate["can_merge"]:
        print("verification blocked:", file=sys.stderr)
        for reason in gate["blocking_reasons"]:
            print(f"  - {reason}", file=sys.stderr)
        return 1
    if args.fail_on_incomplete_coverage and not gate.get("coverage_complete", False):
        print("verification coverage incomplete:", file=sys.stderr)
        for item in list(gate.get("uncovered_checks") or []) + list(gate.get("needs_human_checks") or []):
