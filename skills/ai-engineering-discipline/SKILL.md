...
6. **Verify.** Run native checks (and Semgrep if installed) and read `can_merge` / blocking reasons:
   ...
   ```
7. **Report to the user in plain language only.** Give one short plain message: what you built, whether the tests pass, and anything not covered (e.g. "added delete-by-title with a test; all tests pass; the security scan isn't installed so it didn't run"). Do NOT show the user the generated spec/loop/verify/memory files, `TBD` placeholders, or internal terms like `can_merge`/`coverage_complete` — those are working artifacts, not for the user. Never present "done" as fully verified when `coverage_complete` is false. Update memory only with durable lessons.
