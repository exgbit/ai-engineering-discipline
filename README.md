...
- 测试真的引用了改动的函数 / 类 → 放行(含类内方法:改方法体会归因到该方法,而不是外层类)。
...
- **测试门禁 / 索引是"测试有没有引用改动符号"的检查,不是真覆盖率。** 它拦得住"完全不写测试""空测试""测错函数";但一个引用了改动符号、断言却没意义的测试,理论上还能过。**它防的是无意的偷懒,不是蓄意的绕过。** 同理,索引的"守护"只表示测试文本里出现了符号名,盲区是"很可能没测"的强信号而非证明。**要"改动行真被执行"的硬核查**,加 `--run-diff-coverage`(需覆盖率工具:coverage.py / c8 / go cover / jacoco):它跑覆盖率、和改动行交叉,默认把未覆盖的改动行标注出来;preset 设 `require_diff_coverage`(如 `migration-high`)则硬拦。装了覆盖率工具就根治模块级改动/同名跨文件那几个漏放;没装则降级标注(fail-closed)。**Python / Go / JS 已端到端验证(覆盖率工具真跑、精确定位未覆盖的改动行),Java 解析就绪(配了 jacoco 即生效)。**
