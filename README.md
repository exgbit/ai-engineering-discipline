...
- **测试门禁 / 索引是"测试有没有引用改动符号"的检查,不是真覆盖率。** 它拦得住"完全不写测试""空测试""测错函数";但一个引用了改动符号、断言却没意义的测试,理论上还能过。**它防的是无意的偷懒,不是蓄意的绕过。** 同理,索引的"守护"只表示测试文本里出现了符号名,盲区是"很可能没测"的强信号而非证明。真 diff-coverage 是后续方向(见 `framework/framework-assessment.md`)。
