...
这是流程图里 ② 的核心机制,也是这个框架比较有意思的一步:**重要节点先画成图,再写代码**。它由两条互补的路径组成,所有图统一存在目标项目的 `docs/diagrams/`:

- **框架自动画**:验证时框架用知识图谱(不可用时静态回退)算出本次改动的 blast radius,自动生成 `<名字>-impact.md`——一张 Mermaid 依赖图,**本次改动的符号标橙、受影响但没有测试守护的标红**,调用边能算出来就画出来。你不用画,它是给你"起点"和"验收对照"的。
- **AI 动手画**:feature / refactor / migration 任务会生成 `<名字>-design.md` 占位,AI 必须在编码前把它填成真正的 Mermaid 设计图(受影响/新增的类、函数及调用/依赖关系)——**不画,验证门禁直接亮红灯**。bugfix / docs 等任务默认不强制(preset 分档控制,`verify.require_design_diagram`)。

门禁只判"画了没",不判画得好不好:占位注释删掉了、mermaid 块里有实质节点(只写一行 `flowchart LR` 不算画)才放行;图里合法出现 TBD 备注或复选清单不会误拦。这与框架"防无意偷懒,不防蓄意绕过"的定位一致。

为什么值得:**设计评审被强制前移到写代码之前**——AI 被迫先把"改哪些节点、动哪些关系"想清楚并落成可 review 的图,而不是写完代码再补一张装饰图;图是纯文本 Mermaid,可版本化、可 diff、GitHub 直接渲染;自动 impact 图和手绘设计图放在同一目录互相对照,"计划改什么"与"实际波及什么"一眼可比。上面那张主流程图,就是按这一步要求的画法画的。
