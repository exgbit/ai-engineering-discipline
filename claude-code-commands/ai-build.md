...
4. **Read the code, then fill the spec yourself.** First read the source files the request touches (the scripts do not analyze code). **If a code knowledge-graph MCP is configured (e.g. codebase-memory — see the `impact` layer in `adapters/default-stack.json`), prefer it for impact analysis: index the repo once (`index_repository`), then run `detect_changes` + `explore` on this change to get the affected interfaces and transitive callers, and fill the Impact Analysis from that blast radius — the framework does not verify the graph, so sanity-check it.** Then open the generated `docs/specs/<slug>.md` and complete its `TBD` placeholders (requirements, impact analysis, acceptance criteria, test plan) — give a real Impact Analysis and fill `docs/memory/module-map.md` with the boundaries you found. Never hand `TBD`s back to the user.
